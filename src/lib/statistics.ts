...
export class StatisticsService {
  static generateSummary(
    ...
    const highestAccountBalance =
      accountBalances.length > 0 ? Math.max(...accountBalances) : 0;
    const lowestAccountBalance =
      accountBalances.length > 0 ? Math.min(...accountBalances) : 0;
    ...
    const transactionsLastMonth = transactions.filter((t) => {
      const date = new Date(t.date.toString());
      return (
        (isAfter(date, lastMonth) || date.getTime() === lastMonth.getTime()) &&
        (isBefore(date, endLastMonth) ||
          date.getTime() === endLastMonth.getTime())
      );
    }).length;

    // Category analysis
    const topExpenseCategories = this.calculateCategoryBreakdown(
      expenseTransactions,
      totalExpenses
    );
    const topIncomeCategories = this.calculateCategoryBreakdown(
      incomeTransactions,
      totalIncome
    );
    ...
    const activeRecurring =
      recurringTransactions?.filter((rt) => rt.isActive) || [];
    ...
    const thisYearTransactions = transactions.filter((t) => {
      const date = new Date(t.date.toString());
      return (
        isAfter(date, thisYearStart) ||
        date.getTime() === thisYearStart.getTime()
      );
    });
    ...
    const lastYearTransactions = transactions.filter((t) => {
      const date = new Date(t.date.toString());
      return (
        (isAfter(date, lastYearStart) ||
          date.getTime() === lastYearStart.getTime()) &&
        (isBefore(date, lastYearEnd) ||
          date.getTime() === lastYearEnd.getTime())
      );
    });
    ...
    const daysSinceFirstTransaction =
      this.getDaysSinceFirstTransaction(transactions);
    const averageDailyExpense =
      daysSinceFirstTransaction > 0
        ? totalExpenses / daysSinceFirstTransaction
        : 0;
    const averageDailyIncome =
      daysSinceFirstTransaction > 0
        ? totalIncome / daysSinceFirstTransaction
        : 0;

    // Account distribution by type
    const accountsByType = this.calculateAccountTypeDistribution(
      accounts,
      totalBalance
    );
  ...
  private static getDaysSinceFirstTransaction(
    transactions: Transaction[]
  ): number {
  ...
  private static calculateAccountTypeDistribution(
    ...
    return Object.entries(typeMap)
      .map(([type, data]) => ({
        ...
        percentage:
          totalBalance > 0 ? (data.totalBalance / totalBalance) * 100 : 0,
      }))
