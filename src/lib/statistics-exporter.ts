...
export class StatisticsExporter {
  static exportData(
    ...
    if (options.includeRecurring) {
      data.recurringTransactions = recurringTransactions.map((rt) => ({
        ...rt,
        startDate:
          rt.startDate instanceof Date
            ? rt.startDate.toISOString()
            : rt.startDate,
        endDate:
          rt.endDate instanceof Date ? rt.endDate.toISOString() : rt.endDate,
        lastProcessed:
          rt.lastProcessed instanceof Date
            ? rt.lastProcessed.toISOString()
            : rt.lastProcessed,
        nextProcessDate:
          rt.nextProcessDate instanceof Date
            ? rt.nextProcessDate.toISOString()
            : rt.nextProcessDate,
        createdAt:
          rt.createdAt instanceof Date
            ? rt.createdAt.toISOString()
            : rt.createdAt,
        updatedAt:
          rt.updatedAt instanceof Date
            ? rt.updatedAt.toISOString()
            : rt.updatedAt,
      }));
    }
  ...
  private static convertToText(data: any, options: ExportOptions): string {
    ...
    if (options.includeSummary && data.summary) {
      ...
      text += `Expense to Income Ratio: ${data.summary.expenseToIncomeRatio.toFixed(
        2
      )}\\n`;
      text += "\\n";
      ...
      text += `Highest Account Balance: $${data.summary.highestAccountBalance.toFixed(
        2
      )}\\n`;
      text += `Lowest Account Balance: $${data.summary.lowestAccountBalance.toFixed(
        2
      )}\\n`;
      text += `Average Account Balance: $${data.summary.averageAccountBalance.toFixed(
        2
      )}\\n`;
      text += "\\n";
      ...
      text += `Average Transaction Amount: $${data.summary.averageTransactionAmount.toFixed(
        2
      )}\\n`;
      text += `Average Income Amount: $${data.summary.averageIncomeAmount.toFixed(
        2
      )}\\n`;
      text += `Average Expense Amount: $${data.summary.averageExpenseAmount.toFixed(
        2
      )}\\n`;
      text += `Average Daily Income: $${data.summary.averageDailyIncome.toFixed(
        2
      )}\\n`;
      text += `Average Daily Expense: $${data.summary.averageDailyExpense.toFixed(
        2
      )}\\n`;
      text += "\\n";
      ...
      text += "-".repeat(30) + "\\n";
      text += `This Month Income: $${data.summary.thisMonthIncome.toFixed(
        2
      )}\\n`;
      text += `This Month Expenses: $${data.summary.thisMonthExpenses.toFixed(
        2
      )}\\n`;
      text += `Last Month Income: $${data.summary.lastMonthIncome.toFixed(
        2
      )}\\n`;
      text += `Last Month Expenses: $${data.summary.lastMonthExpenses.toFixed(
        2
      )}\\n`;
      text += "\\n";
      ...
      text += `Estimated Monthly Recurring Income: $${data.summary.estimatedMonthlyRecurringIncome.toFixed(
        2
      )}\\n`;
      text += `Estimated Monthly Recurring Expenses: $${data.summary.estimatedMonthlyRecurringExpenses.toFixed(
        2
      )}\\n`;
      text += "\\n";
      ...
      if (data.summary.topExpenseCategories.length > 0) {
        ...
        data.summary.topExpenseCategories.forEach((cat: any, index: number) => {
          text += `${index + 1}. ${cat.category}: $${cat.amount.toFixed(
            2
      ...
      if (data.summary.accountsByType.length > 0) {
        ...
        data.summary.accountsByType.forEach((type: any) => {
          text += `${type.type}: ${
            type.count
          } accounts, $${type.totalBalance.toFixed(
            2
