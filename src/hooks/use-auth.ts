  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      if (authUser) {
        ...
        if (!userExists) {
          console.error(
            "Auth: User does not exist in Firestore, forcing logout"
          );
        ...
        const dataIntegrity = await verifyUserDataIntegrity(
          authUser.uid,
          authUser.email
        );
        ...
        if (!dataIntegrity) {
          console.error(
            "Auth: User data integrity check failed, forcing logout"
          );
...
export function useRequireAuth(redirectUrl = "/login") {
  ...
  // usePathname/useParams can be null during SSR/hydration; default to safe values
